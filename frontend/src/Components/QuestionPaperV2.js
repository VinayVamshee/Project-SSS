...
export default function QuestionPaperV2() {
        ...
        const loadInitialData = async () => {
            try {
                const [cRes, sRes, csRes, chRes] = await Promise.all([
                    ...
                    axios.get('https://sss-server-eosin.vercel.app/chapters'),
                ]);
    ...
    const fetchQuestions = useCallback(async (cls = selectedClass, subj = selectedSubject, chap = selectedChapter) => {
        if (cls && subj && chap) {
            try {
                const res = await axios.get(`https://sss-server-eosin.vercel.app/questions?class=${cls}&subject=${subj}&chapter=${chap}`);
    ...
    const handleAddQuestion = async () => {
        ...
        try {
            await axios.post('https://sss-server-eosin.vercel.app/questions', {
    ...
    const handleEditSubmit = async () => {
        ...
        setQuestionUploading(true);
        try {
            await axios.put(`https://sss-server-eosin.vercel.app/questions`, {
    ...
    const handleDelete = async (mongoId) => {
        ...
        try {
            await axios.delete('https://sss-server-eosin.vercel.app/questions', {
    ...
    useEffect(() => {
        axios.get('https://sss-server-eosin.vercel.app/get-all-templates').then(res => setTemplates(res.data)).catch(console.error);
    }, []);
